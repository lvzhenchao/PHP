...
<?php
	...
	$SQLForPost = "select *,(select name from userDetail as t2 where t2.id=t1.uid) as userName,(select name from types as t3 where t3.id=t1.tid) as typeName,(select count(*) from reply as t4 where t4.pid=t1.id) as replyCount from post as t1 where id={$_GET['postId']}";
...
#addinfo a{ font-size:14px; font-weight:bold; background:url(../../public/admin/images/main/replayblack.jpg) no-repeat 0 0px; padding:0px 0 0px 20px; line-height:45px;}
#addinfo a:hover{ background:url(../../public/admin/images/main/replayblue.jpg) no-repeat 0 0px;}
</style>
</head>
<body>
<!--main_top-->
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：回复管理</td>
  </tr>
  <tr>
    <td align="left" valign="top" id="addinfo">
    <a href="main_postList.php?postId=<?php echo $_GET['postId']?><?php echo isset($_GET['page']) ? "&page=".$_GET['page'] : '' ;?><?php echo isset($_GET['search']) ? '&search='.$_GET['search'] : '' ;?>" target="mainFrame" onFocus="this.blur()" class="add">返回上一级</a>
    </td>
  </tr>
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        <td align="right" valign="middle" class="borderright borderbottom bggray">帖子相关：</td>
        <td align="left" valign="middle" class="borderright borderbottom main-for">
			<strong>帖子内容：</strong><br />
			<?php echo $resultForPost['title']; ?><br />
			<?php echo $resultForPost['content']; ?>
			<hr />
			<strong>发帖人：</strong><?php echo $resultForPost['userName']; ?>&nbsp;&nbsp;|&nbsp;&nbsp;<strong>时间：</strong><?php echo date($resultForPost['ctime']); ?><br />
			所属板块：<?php echo $resultForPost['typeName']; ?>&nbsp;&nbsp;|&nbsp;&nbsp;回复数量：(<?php echo $resultForPost['replyCount']; ?>)
			
		</td>
        </tr>
		<?php
			$SQLForReply = "select *,(select name from userDetail as t2 where t2.id=t1.uid) as replyName from reply as t1 where pid={$_GET['postId']}";
			$objectForReply = mysqli_query($link,$SQLForReply);
			$replyNum = 1;
			while($resultForReply = mysqli_fetch_assoc($objectForReply)){
			
		?>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        <td align="right" valign="middle" class="borderright borderbottom bggray">回复详情(<?php echo $replyNum++; ?>)：</td>
        <td align="left" valign="middle" class="borderright borderbottom main-for">
			回复时间：<?php echo date('Y-m-d H:i:s',$resultForReply['ctime']) ?><br />
			回复人：<?php echo $resultForReply['replyName']; ?>：<br />
			回复内容：<?php echo $resultForReply['content']; ?><br />
			<p><a href="./delReply.php?postId=<?php echo $_GET['postId']; ?>&replyId=<?php echo $resultForReply['id']; ?>" >删除</a></p>
		</td>
        </tr>
		<?php
		}
